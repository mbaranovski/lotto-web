...
export default class DrawBox extends React.Component {
  ...
  toggleStep = () =>
    this.props.config.toggleable &&
    this.setState(state => ({ step: state.step === "last" ? "next" : "last" }));

  parseDrawingDate = ({ day, month, year }) =>
    new Date(`${month}.${day}.${year}`).toDateString();

  parseWinningNumbers = numbers =>
    numbers
      ? this.props.config.numberSequence
        ? numbers[0].toString().split("")
        : numbers
      : [];
  ...
  render() {
    ...
    return (
      <div
        ...
        className={`draw-box__item ${wide ? "draw-box__item--wide" : ""}`}
      >
        <div className="columns flex draw-box__top" style={{ background }}>
          <div className="draw-box__name">
            <div
              className={`draw-box__title ${
                wide ? "draw-box__title--big" : ""
              }`}
            >
              {title}
            </div>
            <div
              className={`${
                strong ? "draw-box__sub-title--big" : "draw-box__sub-title"
              }`}
            >
              {subTitle}
            </div>
            {time && (
              <div className="draw-box__top__time">
                <i className="draw-box__top__time-icon icon-clock-2" />
                {time}
              </div>
            )}
          </div>
          <div className="flex draw-box__controls">
            <i
              ...
              className="draw-box__arrow icon-arrow-67"
            />
            ...
            <i
              onClick={this.toggleStep}
              className="draw-box__arrow icon-arrow-68"
            />
          </div>
        ...
        <div className="columns flex draw-box__bottom">
          <div
            className={`flex draw-box__bottom__results ${
              wide ? "draw-box__bottom__results--wide" : ""
            }`}
          >
            {numbers && (
              <div
                data-testid="winning-numbers"
                className="draw-box__bottom__results__side"
              >
          ...
          <div className="draw-box__footer">
            {footer &&
              footer.map((item, index) => (
                <a
                  key={`footer-link-${index}-${order}`}
                  className="draw-box__footer__link"
                  href={item.link}
                >
                  {item.name}
                </a>
